...
export interface INeedYouAlert {
  ...
  ackedAt?: string;
  createdAt: string;
  /** True if the current user is the recipient (needs to acknowledge). */
  forMe: boolean;
}
...
/**
 * Get the current user's I Need You recipient preferences (opt-in requirement).
 */
export async function getINeedYouPrefs(userId: string): Promise<INeedYouPrefs> {
  if (isBackendConfigured && supabase) {
    ...
    if (row) {
      return { optIn: row.opt_in, quietHours: row.quiet_hours ?? { enabled: false } };
    }
...
export async function updateINeedYouPrefs(userId: string, prefs: INeedYouPrefs): Promise<void> {
  if (isBackendConfigured && supabase) {
    const client = supabase;
    await client.from('i_need_you_prefs').upsert({
      user_id: userId,
      opt_in: prefs.optIn,
...
/**
 * Send an urgent alert to a connection. The recipient must have opted in
 * (enforced by the insert policy in the schema).
 */
export async function sendINeedYouAlert(
  ...
  if (isBackendConfigured && supabase) {
    ...
    const { error } = await client
      .from('i_need_you')
      .insert({ requester_id: requesterId, recipient_id: recipientId, message: trimmed, status: 'pending' });
    if (error) return { ok: false, error: error.message };
    return { ok: true };
  }

  const recipientName =
    previewAlerts.find((a) => a.recipient.id === recipientId)?.recipient.displayName ?? recipientId;
...
/**
 * Acknowledge an alert sent TO the current user.
 */
...
/**
 * Preview-only: resolve a connection's display name for sending demo alerts.
 */
