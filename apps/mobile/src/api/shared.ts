...
export async function listSpaces(userId: string): Promise<SharedSpace[]> {
  if (isBackendConfigured && supabase) {
    ...
    for (const s of data) {
      const members = ((s.members ?? []) as { profile: unknown }[]).map((m) => toMember(m.profile as unknown as ProfileRow)).filter(Boolean);
      spaces.push({
        id: s.id,
        name: s.name,
...
export async function createSpace(userId: string, name: string, memberIds: string[]): Promise<{ ok: boolean; error?: string }> {
  ...
  if (isBackendConfigured && supabase) {
    ...
    await supabase
      ...
      .insert(ids.map((uid) => ({ space_id: id, user_id: uid })));
    return { ok: true };
  }
...
export async function listMemories(spaceId: string): Promise<Memory[]> {
  if (isBackendConfigured && supabase) {
    const { data, error } = await supabase
      ...
      .order('created_at', { ascending: false });
    if (error || !data) return [];
    const rows = data as Array<{ id: string; space_id: string; added_by: string; note: string; milestone: boolean; media_metadata: MediaMetadata | null; created_at: string; author: unknown }>;
    return rows.map((r) => {
...
export async function listBucketItems(spaceId: string): Promise<BucketItem[]> {
  if (isBackendConfigured && supabase) {
    ...
    if (error || !data) return [];
    return (data as Array<{ id: string; space_id: string; user_id: string; title: string; note: string; done: boolean; done_by: string | null; created_at: string }>).map((r) => ({
      id: r.id,
      spaceId: r.space_id,
      userId: r.user_id,
      title: r.title,
...
export async function toggleBucketItem(spaceId: string, itemId: string, userId: string, done: boolean): Promise<{ ok: boolean; error?: string }> {
  if (isBackendConfigured && supabase) {
