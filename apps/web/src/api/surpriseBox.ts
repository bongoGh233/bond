...
export async function createSurprise(
  ...
  if (isBackendConfigured && supabase) {
    const client = supabase;
    const { error } = await client.from('surprise_boxes').insert({
      sender_id: senderId,
...
export async function openSurprise(boxId: string, userId: string): Promise<{ ok: boolean; content?: string; error?: string }> {
  if (isBackendConfigured && supabase) {
    ...
    const { error: uErr } = await client.from('surprise_boxes').update({ opened: true, opened_at: new Date().toISOString() }).eq('id', boxId);
